...
import { getGeneralTokenUtil } from "./models/roUtil.js";


ac.setAPIKey('79dea53672879a4caec98ef1805e5af5');
ac.getBalance()
    .then(balance => console.log('my balance is: '+balance))
    .catch(error => console.log('an error with API key: '+error));


const login = 'oneAmerican896';
const password = 'insaneclient101';

const proxyAddress = "84.21.190.40"; //only numbers
const proxyPort = "6041"
const proxyUser = "xvcchmky";
const proxyPassword = 'bkmormqke7g6';
...
try {
    ...
    const xfrCatchPromise = new Promise((resolve, reject) => {
        tab.on('response', async (response) => {
            if (response.url() === "https://auth.roblox.com/v2/login" && xfrToken == null){
                ...
                const meta = await response.headers()["rblx-challenge-metadata"];

                const plain = Buffer.from(meta, 'base64').toString('utf8');

                //rblx_challenge_id = err.response.headers["rblx-challenge-id"];
                //rblx_challenge_metadata = err.response.headers["rblx-challenge-metadata"];
                ...
                console.log(json);
                const dataBlob = plain;

                const xfrToken = await getGeneralTokenUtil();
    ...
    console.log(dataBlobObject);
    const token = await solveArkoselabsCaptcha(dataBlobObject.dataExchangeBlob);
